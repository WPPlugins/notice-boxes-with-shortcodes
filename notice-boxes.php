...
function noticeboxes($atts, $content = null){extract(shortcode_atts(array("type" => 'notice',"align" => 'left',"actualcodetooutput" => '',"close" => 'no'), $atts));$alignstring = 'noticeboxesde';
    if ($align == 'center') {$alignstring = 'noticeboxesc';}if ($type == 'notice') {$type = 'bluenoticebox';}elseif ($type == 'info') {$type = 'bluenoticebox';}elseif ($type == 'success') {$type = 'greennoticebox';}elseif ($type == 'warning') {$type = 'yellownoticebox';}elseif ($type == 'error') {$type = 'rednoticebox';}$actualcodetooutput = '<div class="' . $alignstring . ' ' . $type . '">' . $content . '';
